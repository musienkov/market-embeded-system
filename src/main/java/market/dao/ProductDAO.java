...
public interface ProductDAO extends CrudRepository<Product, Long>, JpaRepository<Product, Long> {
	...
	Page<Product> findByDistilleryOrderByName(Category distillery, Pageable request);

	@Query(value = "SELECT p FROM Product p WHERE p.distillery IN (SELECT d FROM Category d WHERE d.region = :region) order by p.name")
